                                     ...
                                     double excess_limit)
{
  ...
  f_t arc_distance =
    get_distance(curr_node.request.info, fragment[0].request.info, s_route.vehicle_info());
  fragment[fragment_size - 1].cost_dim.cost_forward =
    curr_node.cost_dim.cost_forward + arc_cost + fragment_cost;
  fragment[fragment_size - 1].cost_dim.distance_forward =
    curr_node.cost_dim.distance_forward + arc_distance + fragment_distance;
...
DI bool backward_fragment_update_cvrp(const node_t<i_t, f_t, REQUEST>& curr_node,
                                      ...
                                      double excess_limit)
{
  f_t arc_cost = get_arc_cost(
    fragment[fragment_size - 1].request.info, curr_node.request.info, s_route.vehicle_info());
  f_t arc_distance = get_distance(
    fragment[fragment_size - 1].request.info, curr_node.request.info, s_route.vehicle_info());
  fragment[0].cost_dim.cost_backward = curr_node.cost_dim.cost_backward + arc_cost + fragment_cost;
  fragment[0].cost_dim.distance_backward =
    curr_node.cost_dim.distance_backward + arc_distance + fragment_distance;
